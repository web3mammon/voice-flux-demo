  ...
  return <div className="min-h-screen flex flex-col p-6 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto flex-1 flex flex-col">
        ...
        <div className="text-center space-y-4 py-8 animate-fade-in">
          <h1 className="text-6xl tracking-tight bg-gradient-to-r from-primary via-secondary to-primary bg-clip-text text-transparent font-extrabold">Say Hello to Jennifer</h1>
          <p className="text-muted-foreground text-lg font-light">NLC's new all round assistant by  Klariqo</p>
        </div>

        {/* Main content area */}
        <div className="flex-1 flex items-center justify-center">
          {messages.length === 0 && !currentAssistantMessage ? (/* Show orb when no conversation */
        <VoiceOrb state={state} />) : (/* Show chat transcript when conversation started */
        <div className="w-full max-w-4xl">
              <ChatTranscript messages={messages} currentAssistantMessage={currentAssistantMessage} state={state} />
            </div>)}
        </div>
        ...
        <div className="flex flex-col items-center gap-6 py-8">
          <div className="flex justify-center gap-4">
            {!isActive ? <Button onClick={startConversation} size="lg" className="bg-gradient-to-r from-primary to-secondary hover:opacity-90 transition-all duration-300 text-white px-10 py-7 text-lg font-light shadow-2xl hover:shadow-primary/50 rounded-full">
                <Mic className="mr-3 h-6 w-6" />
                Start Conversation
              </Button> : <Button onClick={stopConversation} size="lg" variant="destructive" className="px-10 py-7 text-lg font-light shadow-2xl rounded-full">
